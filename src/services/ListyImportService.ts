...
export class ListyImportService {
	...
	async processListyData(listyData: ListyData): Promise<number> {
		...
		if (
			...
			this.settings.outputFolder.length > 0
		) {
		...
		for (const list of listyData.lists) {
			...
			const shouldConsolidate =
				isToDoList && this.settings.consolidateToDoLists;

			if (shouldConsolidate) {
				await this.createConsolidatedToDoList(
					list,
					this.settings.outputFolder
				);
				totalNotesCreated++;
			...
			const listFolderPath = normalizePath(
				`${this.settings.outputFolder}/${sanitizedListTitle}`
			);
			...
			for (const item of list.items) {
				...
				const notePath = normalizePath(
					`${listFolderPath}/${sanitizedItemTitle}.md`
				);

				let existingContent: string | null = null;
				try {
					existingContent = await this.app.vault.adapter.read(
						notePath
					);
					...
					if (this.settings.enableNoteLocking && existingContent) {
						const lockMatch =
							existingContent.match(/^lock:\s*true/m);
						if (lockMatch) {
							console.log(
								`Skipping locked note: ${sanitizedItemTitle}`
							);
							continue;
						}
					}
					...
					console.log(
						`Updating existing note: ${sanitizedItemTitle}`
					);
				} catch (error) {
				...
				const noteContent = await this.generateNoteContent(
					item,
					list,
					existingContent
				);
	...
	private async generateNoteContent(
		...
		if (existingContent) {
			...
			if (itemId) {
				...
				return this.updateExistingNote(
					item,
					list,
					existingContent,
					itemId
	...
	private async updateExistingNote(
		...
		if (
			!existingContent.includes(startBlockMarker) ||
		...
		const blockEndIndex =
			existingContent.indexOf(endBlockMarker) + endBlockMarker.length;
		...
		let result = frontmatter;
		result +=
			startBlockMarker + beforeContent + startContentMarker + "\n\n";
	...
	private generateFrontmatter(item: ListyItem, list: ListyList): string {
		...
		if (item.attributes && item.attributes.length > 0) {
			...
			if (attributesMap.has("description")) {
				const processedDesc = this.processDescription(
					attributesMap.get("description") || ""
				);
			...
			if (attributesMap.has("author")) {
				frontmatter += `author: "${this.escapeYamlString(
					attributesMap.get("author") || ""
				)}"\n`;
				attributesMap.delete("author");
			}

			if (attributesMap.has("cover")) {
				frontmatter += `cover: "${this.escapeYamlString(
					attributesMap.get("cover") || ""
				)}"\n`;
			...
			if (attributesMap.has("tags")) {
				...
				if (this.settings.includeTags) {
					...
					if (tags.length > 0) {
						...
						for (const tag of tags) {
							frontmatter += `  - "${this.escapeYamlString(
								tag
							)}"\n`;
						}
					}
				...
				frontmatter += `original_tags: "${this.escapeYamlString(
					attributesMap.get("tags") || ""
				)}"\n`;
				attributesMap.delete("tags");
			}

			if (attributesMap.has("icon")) {
				frontmatter += `icon: "${this.escapeYamlString(
					attributesMap.get("icon") || ""
				)}"\n`;
			...
			for (const [key, value] of attributesMap.entries()) {
				if (value.includes("\n") || value.length > 80) {
					...
					frontmatter += `${key}: "${this.escapeYamlString(
						value
					)}"\n`;
				}
			}
	...
	private async generateItemContent(
		...
		sanitizedItem.title = this.sanitizeFileName(item.title).substring(
			0,
			maxLength
		);
		if (sanitizedItem.title.length === maxLength) {
			sanitizedItem.title =
				sanitizedItem.title.substring(0, maxLength - 3) + "...";
		}
		...
		return applyTemplateTransformations(
			templateContent,
			sanitizedItem,
			list
		);
	 ...
	 */
	private async createConsolidatedToDoList(
		list: ListyList,
		folderPath: string
	): Promise<void> {
		const listPath = normalizePath(
			`${folderPath}/${this.sanitizeFileName(list.title)}.md`
		);
		...
		const listId = existingContent
			? this.extractItemId(existingContent)
			: uuidv4();
		...
		for (const item of list.items) {
			const isCompleted =
				item.marked ||
				(existingTasks.has(item.title) &&
					existingTasks.get(item.title));
