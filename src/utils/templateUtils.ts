...
export async function getTemplateContents(
	...
	try {
		...
		console.error(
			`Failed to read the Listy template '${normalizedTemplatePath}'`,
			err
		);
...
export function applyTemplateTransformations(
	...
	if (item.attributes && item.attributes.length > 0) {
		const description = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "DESCRIPTION"
		);
		...
		const cover = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "COVER"
		);
		result = result.replace(/{{Cover}}/gi, cover ? cover.value : "");

		const userNote = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "USER_NOTE"
		);
		result = result.replace(
			/{{UserNote}}/gi,
		...
		const author = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "AUTHOR"
		);
		result = result.replace(/{{Author}}/gi, author ? author.value : "");

		const genre = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "GENRE"
		);
		result = result.replace(/{{Genre}}/gi, genre ? genre.value : "");

		const rating = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "RATING"
		);
		result = result.replace(/{{Rating}}/gi, rating ? rating.value : "");

		const date = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "DATE"
		);
		result = result.replace(/{{Date}}/gi, date ? date.value : "");

		const price = item.attributes.find(
			(attr) => attr.key.toUpperCase() === "PRICE"
		);
